...
local npcDropdown = Tabs.Teleport:AddDropdown("NpcDropdown", {
    ...
    -- 🔥 **ใช้ _G.selectedBosses ถ้ามีการกำหนดไว้ มิฉะนั้นใช้ค่าพื้นฐาน**
    ...
    local function hopServer()
        ...
        if not success then
            print("⚠️ เกิดข้อผิดพลาดในการโหลดเซิร์ฟเวอร์! ลองใหม่อีกครั้งใน 10 วินาที...")
            task.wait(10)
            return hopServer()
        end
        ...
        for _, server in pairs(servers.data) do
            if server.playing < server.maxPlayers and server.id ~= currentJobId then
                print("🔀 กำลังย้ายไปเซิร์ฟเวอร์ใหม่...")
                task.wait(3) -- รอให้เซิร์ฟเวอร์โหลดก่อนกระโดด
                TeleportService:TeleportToPlaceInstance(placeId, server.id)
                return
            end
        end

        print("⚠️ ไม่มีเซิร์ฟเวอร์ใหม่ ลองใหม่อีกครั้งใน 10 วินาที...")
        task.wait(10)
        hopServer()
    end
    ...
    local function autoFarmBosses()
        while autoFarmEnabled do
            ...
            -- 🔥 **ถ้าไม่พบบอสเลย ให้ Hop Server ทันที**
            if not foundBoss then
                print("⚠️ ไม่พบบอสที่ต้องการในเซิร์ฟเวอร์นี้ กำลัง Hop Server...")
                task.wait(3) -- รอให้ระบบ Hop Server ทำงานก่อนวนลูปใหม่
                hopServer()
            else
